          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-8 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-12 border-t pt-8">
                ...
                <div className="flex gap-2 mb-6">
                    ...
                    <button onClick={handleUploadImage} className="bg-green-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-green-700">Upload</button>
                </div>
                <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
                    {dbImages.map((img) => (
                        <div key={img._id} className="relative group aspect-square">
                            ...
                            <button onClick={() => deleteImage(img._id)} className="absolute -top-2 -right-2 bg-red-500 text-white w-7 h-7 rounded-full opacity-0 group-hover:opacity-100 flex items-center justify-center shadow-lg font-bold">×</button>
                        </div>
                    ))}
                </div>
            </div>
      ...
      <Routes>
        <Route path="/" element={
          <section className="relative h-[80vh] flex items-center justify-center text-center px-6">
            <div>
              ...
              <Link to="/order" className="px-12 py-5 bg-purple-600 text-white rounded-full font-black shadow-xl hover:bg-purple-700 transition-all">Order Now</Link>
            </div>
          </section>
        } />
        ...
        <Route path="/gallery" element={
          <section className="py-20 px-6 max-w-7xl mx-auto text-center">
            <h2 className="text-5xl font-black mb-16 italic text-gray-900 tracking-tight">Gallery</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {dbImages.map((img, i) => (
                <div key={i} className="aspect-[3/4] rounded-3xl overflow-hidden shadow-xl hover:scale-[1.03] transition-all">
                   <img src={img.imageUrl} className="w-full h-full object-cover" alt="Gallery Item" />
                </div>
              ))}
              {[img1, img2, img3, img4, imga, imgb].map((pic, i) => (
                <div key={`st-${i}`} className="aspect-[3/4] rounded-3xl overflow-hidden shadow-xl hover:scale-[1.03] transition-all">
        ...
        {/* --- Success Page አስተካክለነዋል --- */}
        <Route path="/success" element={
          <div className="h-screen flex flex-col items-center justify-center bg-green-50 text-center px-6">
            <div className="p-12 bg-white rounded-[50px] shadow-2xl">
              <h1 className="text-9xl mb-6">✅</h1>
              <h2 className="text-4xl font-black text-green-600 mb-4 tracking-tighter">ክፍያዎ ተፈጽሟል!</h2>
              <p className="text-gray-500 font-bold mb-10">ትዕዛዝዎን በትክክል ተቀብለናል፣ እናመሰግናለን።</p>
              <Link to="/" className="px-10 py-4 bg-purple-600 text-white rounded-2xl font-black shadow-lg hover:bg-purple-700 transition-all">
                ወደ ዋና ገጽ ተመለስ
              </Link>
            </div>
          </div>
  ...
  return (
    <section className="py-20 px-6 min-h-screen flex items-center justify-center bg-purple-50">
      <div className="max-w-md w-full bg-white p-12 rounded-[40px] shadow-2xl">
        <h2 className="text-4xl font-black mb-10 text-center italic tracking-tight text-purple-700">አዲስ ትዕዛዝ</h2>
        <form onSubmit={handleOrderSubmit} className="space-y-6">
          <input type="text" placeholder="ሙሉ ስም" value={order.name} onChange={(e) => setOrder({...order, name: e.target.value})} className="w-full p-5 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold" required />
          <input type="text" placeholder="የልብስ አይነት" value={order.orderType} onChange={(e) => setOrder({...order, orderType: e.target.value})} className="w-full p-5 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold" required />
          <button type="submit" className="w-full py-5 bg-purple-600 text-white rounded-2xl font-black text-lg shadow-xl hover:bg-purple-700 transition-all" disabled={loading}>

        ...
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4 backdrop-blur-md" onClick={() => setShowAdmin(false)}>
          <div className="bg-gray-900/40 backdrop-blur-3xl rounded-[40px] w-full max-w-4xl max-h-[85vh] overflow-y-auto p-10 shadow-2xl border border-white/10 text-white" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-8 pt-10 border-t border-white/10">
                <h3 className="text-xl font-black mb-6 text-purple-300 italic">Content Management</h3>
                <div className="flex gap-4 mb-8">
                    ...
                    <button onClick={handleUploadImage} className="bg-purple-600 text-white px-8 py-2 rounded-2xl font-black hover:bg-purple-700 transition-all shadow-lg">Upload</button>
                </div>
                <div className="grid grid-cols-4 gap-2">
                    {dbImages.map((img) => (
                        <div key={img._id} className="relative group aspect-square rounded-xl overflow-hidden border border-white/10 shadow-lg bg-black">
                            {isVideo(img.imageUrl) ? (
                              <video src={img.imageUrl} className="w-full h-full object-cover opacity-50" />
                            ) : (
        ...
        <Routes>
          ...
          <Route path="/gallery" element={
            <section className="py-12 px-4 max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-4 gap-2 md:gap-8 animate-fade-in-up mb-24">
              ...
              {/* Video Section (Static & Dynamic) */}
              <div className="text-center mb-16 animate-fade-in-up">
                  <h2 className="text-4xl md:text-6xl font-black italic text-white tracking-tighter mb-4 drop-shadow-lg text-purple-100 uppercase">Videos</h2>
                  <div className="h-2 w-20 bg-purple-500 mx-auto rounded-full shadow-lg shadow-purple-500/50"></div>
              </div>
              <div className="grid grid-cols-4 gap-2 md:gap-8 animate-fade-in-up">
                {/* 1. Dynamic Videos from DB */}
                {dbImages.filter(item => isVideo(item.imageUrl)).map((vid, i) => (
                  <div key={i} className="aspect-[3/4] rounded-xl md:rounded-[45px] overflow-hidden shadow-2xl hover:-translate-y-2 transition-all duration-700 ring-1 ring-white/20 backdrop-blur-sm bg-white/5">
                     <video src={vid.imageUrl} className="w-full h-full object-cover" controls preload="metadata" />
                  </div>
                ))}
                {/* 2. Static Videos from Assets */}
                {[vid1, vid2, vid3, vid4, vid5, vid7, vid8, vid10].map((video, i) => (
                  <div key={`st-vid-${i}`} className="aspect-[3/4] rounded-xl md:rounded-[45px] overflow-hidden shadow-2xl hover:-translate-y-2 transition-all duration-700 ring-1 ring-white/20 backdrop-blur-sm bg-white/5">
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
            ...
            <div className="flex flex-row items-center gap-8 border-t md:border-t-0 border-white/10 pt-8 md:pt-0">
              ...
              <div className="flex flex-row gap-3">
                <a href="https://t.me/Lilmoo_design13" target="_blank" rel="noreferrer" className="w-10 h-10 md:w-12 md:h-12 bg-white/10 hover:bg-purple-600 rounded-2xl flex items-center justify-center text-white transition-all border border-white/10 shadow-lg group"><span className="font-black text-[10px] group-hover:scale-110 transition-transform">TG</span></a>
                <a href="https://instagram.com/yomu_ko" target="_blank" rel="noreferrer" className="w-10 h-10 md:w-12 md:h-12 bg-white/10 hover:bg-pink-600 rounded-2xl flex items-center justify-center text-white transition-all border border-white/10 shadow-lg group"><span className="font-black text-[10px] group-hover:scale-110 transition-transform">IG</span></a>
              </div>
            </div>
          </div>
    ...
    <section className="min-h-[80vh] flex items-center justify-center px-6">
      <div className="max-w-md w-full bg-white/10 backdrop-blur-3xl p-10 md:p-14 rounded-[40px] md:rounded-[60px] shadow-[0_40px_100px_rgba(0,0,0,0.5)] border border-white/20 animate-fade-in-up text-white">
        ...
        <form onSubmit={handleOrderSubmit} className="space-y-6">
          <input type="text" placeholder="ሙሉ ስም" value={order.name} onChange={(e) => setOrder({...order, name: e.target.value})} className="w-full p-5 bg-white/10 border border-white/10 focus:border-purple-400 focus:bg-white/20 rounded-[25px] outline-none font-bold text-white placeholder-white/30" required />
          <input type="text" placeholder="የልብስ አይነት" value={order.orderType} onChange={(e) => setOrder({...order, orderType: e.target.value})} className="w-full p-5 bg-white/10 border border-white/10 focus:border-purple-400 focus:bg-white/20 rounded-[25px] outline-none font-bold text-white placeholder-white/30" required />
          <button type="submit" className="w-full py-5 bg-purple-600 text-white rounded-[25px] font-black text-xl shadow-2xl hover:bg-purple-700 transition-all ring-1 ring-white/20" disabled={loading}>

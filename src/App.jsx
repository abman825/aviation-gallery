...
// Static Videos (ከAssets ፎልደር የመጡ)
...
export default function App() {
  ...
  const handleUploadImage = async () => {
    if(!selectedFile) return alert("እባክህ መጀመሪያ ፋይል (ፎቶ ወይም ቪዲዮ) ምረጥ");
  ...
  const deleteImage = async (id) => {
    if(window.confirm("ይህ ፋይል እንዲጠፋ ትፈልጋለህ?")) {
          ...
          <div className="bg-gray-900/40 backdrop-blur-3xl rounded-[40px] w-full max-w-4xl max-h-[85vh] overflow-y-auto p-10 shadow-2xl border border-white/10 text-white" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-8 pt-10 border-t border-white/10">
                ...
                <div className="grid grid-cols-4 gap-2">
                    {dbImages.map((img) => (
                        <div key={img._id} className="relative group aspect-square rounded-xl overflow-hidden border border-white/10 shadow-lg bg-black/20">
        ...
        <Routes>
          <Route path="/" element={
            <section className="min-h-[75vh] flex items-center justify-center px-6">
              <div className="w-full max-w-4xl p-14 md:p-24 bg-white/10 backdrop-blur-3xl border border-white/20 rounded-[40px] md:rounded-[60px] shadow-[0_40px_100px_rgba(0,0,0,0.4)] text-center animate-fade-in-up">
                ...
                <p className="text-sm md:text-xl text-gray-200 mb-12 font-medium max-w-xl mx-auto leading-relaxed italic">ልዩ ዲዛይኖችን በጥራት እናቀርባለን። ዘመናዊነትን ከጥራት ጋር አጣምረን ለእናንተ።</p>
          ...
          <Route path="/gallery" element={
            <section className="py-12 px-4 max-w-7xl mx-auto">
              ...
              {/* Images Grid */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 animate-fade-in-up mb-12">
              ...
              <div className="text-center mb-10">
                  <h3 className="text-2xl font-black italic text-purple-300 uppercase">Cinematics</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in-up">
                {[vid1, vid2, vid3, vid4].map((video, i) => (
                  <div key={`vid-${i}`} className="aspect-video rounded-[30px] overflow-hidden border border-white/10 bg-black/40 shadow-2xl">
                ...
                {dbImages.filter(img => isVideo(img.imageUrl)).map((v, i) => (
                   <div key={`dbvid-${i}`} className="aspect-video rounded-[30px] overflow-hidden border border-white/10 bg-black/40 shadow-2xl">
                     <video src={v.imageUrl} className="w-full h-full object-cover" controls preload="metadata" />
                  </div>
                ))}
              </div>
            </section>
          ...
          <Route path="/success" element={
            <div className="h-[70vh] flex flex-col items-center justify-center text-center px-6">
              <div className="p-10 md:p-20 bg-white/10 backdrop-blur-3xl rounded-[40px] md:rounded-[70px] border border-white/20 shadow-2xl animate-fade-in-up text-white">
                ...
                <h1 className="text-3xl md:text-5xl font-black mb-6 tracking-tighter italic">ክፍያዎ ተፈፅሟል!</h1>
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
            <div className="text-center md:text-left flex-1">
              ...
              <p className="text-gray-200 text-xs md:text-sm font-medium leading-relaxed max-w-sm italic mt-2">
                ሊልሙ ዲዛይን የዘመናዊ ልብስ ጥበብን ከአዲስ እይታ ጋር ያቀርባል። በጥራት እና በዘመናዊነት የታነፁ ልዩ ስራዎች።
              </p>
            </div>

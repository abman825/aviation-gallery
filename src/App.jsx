...
export default function App() {
  ...
  // 💡 Localhost ላይ ከሆንክ 10000 ተጠቀም፣ Render ላይ ከሆነ የሰርቨርህን ሊንክ ተጠቀም
  const API_URL = "https://aviation-backend-g75i.onrender.com"; 
  ...
  // 2. አድሚን ገጽ ለመክፈት (Orders & Gallery Management)
  ...
  const deleteImage = async (id) => {
    if(window.confirm("ፎቶው እንዲጠፋ ትፈልጋለህ?")) {
      ...
      {showAdmin && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowAdmin(false)}>
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-8 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {/* Orders Section */}
            <h3 className="text-xl font-bold mb-4 text-purple-700">Recent Orders</h3>
                ...
                <tbody className="divide-y">
                  {adminOrders.map((ord) => (
                    <tr key={ord._id} className="text-sm">
                      ...
                      <td className="p-4"><button onClick={() => deleteOrder(ord._id)} className="text-red-500 hover:underline">Delete</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="border-t pt-8">
              <h3 className="text-xl font-bold mb-4 text-purple-700">Manage Gallery Images</h3>
              <div className="flex gap-2 mb-6">
                <input type="text" placeholder="Paste Image URL here..." value={imgUrl} onChange={(e) => setImgUrl(e.target.value)} className="flex-1 p-3 border rounded-xl outline-none" />
